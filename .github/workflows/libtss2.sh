#!/bin/bash

#----libtss2.sh----
#
# This script runs the tpm2-tss tests against the FIPS wolfProvider.
# Environment variables TPM2_TSS_REF, WOLFSSL_REF, and OPENSSL_REF
# are set by Jenkins.
set -e
set -x

# Use stable version instead of specific commit
TPM2_TSS_REF="4.1.3"

# Define base directories for cleaner paths
WOLFPROV_DIR="/home/user/wolfProvider"
WOLFSSL_INSTALL="$WOLFPROV_DIR/wolfssl-install"
OPENSSL_INSTALL="$WOLFPROV_DIR/openssl-install"
WOLFPROV_INSTALL="$WOLFPROV_DIR/wolfprov-install"

# Go to wolfProvider directory
cd "$WOLFPROV_DIR"

# Clone tpm2-tss repo
rm -rf tpm2_tss_repo
git clone --depth=1 --branch="${TPM2_TSS_REF}" https://github.com/tpm2-software/tpm2-tss.git tpm2_tss_repo
cd tpm2_tss_repo

# Set environment variables
export LD_LIBRARY_PATH="${WOLFSSL_INSTALL}/lib:${OPENSSL_INSTALL}/lib64"
export OPENSSL_CONF="${WOLFPROV_DIR}/provider-fips.conf"
export OPENSSL_MODULES="${WOLFPROV_INSTALL}/lib"
export PKG_CONFIG_PATH="${OPENSSL_INSTALL}/lib64/pkgconfig"
export LDFLAGS="-L${OPENSSL_INSTALL}/lib64"
export CPPFLAGS="-I${OPENSSL_INSTALL}/include"

patch -p1 < ../tpm2_tss_fips_patch.diff

# Apply patch to fix missing stdint.h includes in test files
if [ "$TPM2_TSS_REF" = "4.1.3" ]; then
  perl -pi -e 's|(#include <cmocka\.h>)|#include <stdint.h>\n$1|' ./test/unit/*.c
fi

# Set up the environment for wolfProvider (equivalent to source scripts/env-setup)
export LD_LIBRARY_PATH="${WOLFSSL_INSTALL}/lib:${OPENSSL_INSTALL}/lib64:$LD_LIBRARY_PATH"

# Build and install tpm2-tss
./bootstrap
./configure --prefix=$PWD/tpm2-tss-install --with-crypto=ossl \
  --enable-unit --includedir="${OPENSSL_INSTALL}/include" \
  --enable-wolfprov-fips
make -j$(nproc)
make install

# Run tpm2-tss tests
make check 

if [ $? -eq 0 ]; then
  echo "Workflow completed successfully"
else
  echo "Workflow failed"
  exit 1
fi
