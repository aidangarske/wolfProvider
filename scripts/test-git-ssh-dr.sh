                ...
                "clone")
                    ...
                    if git clone --verbose "$TEST_BASE_DIR/test-repo.git" cloned-repo 2>&1 | tee -a "$error_log"; then
                        if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                            status="SUCCESS"
                            ((success_count++))
                            print_status "FAILURE" "Clone successful (with WPFF=1 - wolfProvider is being used!)"
                            check_force_fail
                        else
                            status="SUCCESS"
                            ((success_count++))
                    ...
                    if [ -d "cloned-repo" ]; then
                        ...
                        if timeout 30 git push origin main 2>>"$error_log"; then
                            if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                                status="SUCCESS"
                                ((success_count++))
                                print_status "FAILURE" "Push successful (with WPFF=1 - wolfProvider is being used!)"
                                check_force_fail
                            else
                                status="SUCCESS"
                                ((success_count++))
                    ...
                    if [ -d "cloned-repo" ]; then
                        ...
                        if timeout 30 git pull origin main 2>>"$error_log"; then
                            if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                                status="SUCCESS"
                                ((success_count++))
                                print_status "FAILURE" "Pull successful (with WPFF=1 - wolfProvider is being used!)"
                                check_force_fail
                            else
                                status="SUCCESS"
                                ((success_count++))
                    ...
                    if [ -d "cloned-repo" ]; then
                        ...
                        if timeout 30 git fetch origin 2>>"$error_log"; then
                            if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                                status="SUCCESS"
                                ((success_count++))
                                print_status "FAILURE" "Fetch successful (with WPFF=1 - wolfProvider is being used!)"
                                check_force_fail
                            else
                                status="SUCCESS"
                                ((success_count++))
            ...
            "rsa")
                if ssh-keygen -t rsa -b 4096 -f "$ssh_key" -N "" -C "test-rsa-key-$attempt" 2>/dev/null; then
                    if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                        ...
                        ((success_count++))
                        print_status "FAILURE" "RSA key generation successful (with WPFF=1 - wolfProvider is being used!)"
                        check_force_fail
                    else
                        key_gen_status="SUCCESS"
                        ((success_count++))
            ...
            "ecdsa")
                if ssh-keygen -t ecdsa -b 521 -f "$ssh_key" -N "" -C "test-ecdsa-key-$attempt" 2>/dev/null; then
                    if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                        key_gen_status="SUCCESS"
                        ((success_count++))
                        print_status "FAILURE" "ECDSA key generation successful (with WPFF=1 - wolfProvider is being used!)"
                        check_force_fail
                    else
                        key_gen_status="SUCCESS"
                        ((success_count++))
            ...
            "ed25519")
                if ssh-keygen -t ed25519 -f "$ssh_key" -N "" -C "test-ed25519-key-$attempt" 2>/dev/null; then
                    if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                        key_gen_status="SUCCESS"
                        ((success_count++))
                        print_status "FAILURE" "ED25519 key generation successful (with WPFF=1 - wolfProvider is being used!)"
                        check_force_fail
                    else
                        key_gen_status="SUCCESS"
                        ((success_count++))
            ...
            "default")
                ...
                if ssh-keygen -t rsa -f "$ssh_key" -N "" -C "test-default-key-$attempt" 2>/dev/null; then
                    if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                        key_gen_status="SUCCESS"
                        ((success_count++))
                        print_status "FAILURE" "Default SSH key generation successful (with WPFF=1 - wolfProvider is being used!)"
                        check_force_fail
                    else
                        key_gen_status="SUCCESS"
                        ((success_count++))
                    ...
                    "clone")
                        ...
                        if git clone --verbose "$TEST_BASE_DIR/test-repo.git" cloned-repo 2>&1 | tee -a "$error_log"; then
                            if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                                status="SUCCESS"
                                ((success_count++))
                                print_status "FAILURE" "Git clone with $key_type key successful (with WPFF=1 - wolfProvider is being used!)"
                                check_force_fail
                            else
                                status="SUCCESS"
                                ((success_count++))
                        ...
                        if [ -d "cloned-repo" ]; then
                            ...
                            if timeout 30 git push origin main 2>>"$error_log"; then
                                if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                                    status="SUCCESS"
                                    ((success_count++))
                                    print_status "FAILURE" "Git push with $key_type key successful (with WPFF=1 - wolfProvider is being used!)"
                                    check_force_fail
                                else
                                    status="SUCCESS"
                                    ((success_count++))
                        ...
                        if [ -d "cloned-repo" ]; then
                            ...
                            if timeout 30 git pull origin main 2>>"$error_log"; then
                                if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                                    status="SUCCESS"
                                    ((success_count++))
                                    print_status "FAILURE" "Git pull with $key_type key successful (with WPFF=1 - wolfProvider is being used!)"
                                    check_force_fail
                                else
                                    status="SUCCESS"
                                    ((success_count++))
                        ...
                        if [ -d "cloned-repo" ]; then
                            ...
                            if timeout 30 git fetch origin 2>>"$error_log"; then
                                if [ "${WOLFPROV_FORCE_FAIL}" = "1" ]; then
                                    status="SUCCESS"
                                    ((success_count++))
                                    print_status "FAILURE" "Git fetch with $key_type key successful (with WPFF=1 - wolfProvider is being used!)"
                                    check_force_fail
                                else
                                    status="SUCCESS"
                                    ((success_count++))
