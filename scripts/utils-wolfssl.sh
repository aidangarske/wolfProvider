...
# pwdbased is off by default and PBES2 key encryption needs it, so without it
# the provider cannot honour a cipher on a private-key encoder.
WOLFSSL_FIPS_CONFIG_OPTS=${WOLFSSL_CONFIG_OPTS:-'--enable-opensslcoexist --enable-pwdbased '}
